...
class UrmpMidi(Urmp):
  ...
  def get_dataset(self, shuffle=True):
    ...
    def _reshape_tensors(data):
      ...
      onsets = tf.reduce_sum(tf.reshape(
          data['note_onsets'], (-1, 128)), axis=-1)
      data['onsets'] = tf.cast(onsets > 0, tf.int64)
      offsets = tf.reduce_sum(tf.reshape(
        data['note_offsets'], (-1, 128)), axis=-1)
...
class UrmpMidiUnbatched(Urmp):
  """Urmp training set with midi note data.
  Loads unbatched dataset with variable length data.
  """
