...
"""Layer wrapper for streaming inference.

Directly ported from: https://github.com/google-research/google-research
/blob/master/kws_streaming/layers/stream.py
"""
...
class Stream(tfkl.Layer):
  ...
  def __init__(self,
    ...
    if not use_one_step and isinstance(
        wrappped_cell,
        (tfkl.Flatten, tfkl.GlobalMaxPooling2D,
    ...
    if self.ring_buffer_size_in_time_dim is not None:
      ...
      pass
        ...
        wrappped_cell,
        (tfkl.Conv1D, tfkl.Conv2D,
         tfkl.DepthwiseConv2D, tfkl.SeparableConv2D)):
    ...
    elif isinstance(wrappped_cell, tfkl.AveragePooling2D):
      strides = wrappped_cell.get_config()['strides']
      pool_size = wrappped_cell.get_config()['pool_size']
      self.stride = strides[0]
      if self.mode not in (
          Modes.TRAINING,
          Modes.NON_STREAM_INFERENCE) and strides[0] != pool_size[0]:
  ...
  def build(self, input_shape):
    ...
    if isinstance(
      ...
      self.state_shape = [
        ...
        wrappped_cell,
        (tfkl.Flatten, tfkl.GlobalMaxPooling2D,
         tfkl.GlobalAveragePooling2D)) and not self.state_shape:
...
# --- Streaming Layer Factories --------
def streaming_conv_1d(ch,
                      k,
                      stride=1,
                      dilation=1,
                      training=True,
                      inference_batch_size=1):
  """Make a streaming version of 1-D conv layer."""
  mode = Modes.TRAINING if training else Modes.STREAM_INTERNAL_STATE_INFERENCE
